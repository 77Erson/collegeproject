...
import { Chrome, GraduationCap, User } from 'lucide-react';
...
const LoginPage = () => {
  ...
  const { register: registerStudent, handleSubmit: handleSubmitStudent, formState: { errors: studentErrors } } = useForm<StudentLoginFormData>();
  ...
  const onStudentSubmit = async (data: StudentLoginFormData) => {
    ...
    // In a real app, you'd verify the student ID and password against the database
    toast({
      title: "Student login successful",
      description: `Welcome back, ${data.studentId}!`,
    });
          ...
          <CardContent>
            <Tabs defaultValue="teacher" className="w-full">
              ...
              <TabsContent value="student" className="space-y-4 mt-4">
                <form onSubmit={handleSubmitStudent(onStudentSubmit)} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    {studentErrors.studentId && (
                      <p className="text-sm text-red-500">{studentErrors.studentId.message}</p>
                    )}
                  </div>
                  <div className="space-y-2">
